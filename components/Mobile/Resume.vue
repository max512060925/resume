...
.mobile-resume.w-screen.h-screen.scroll-smooth.overflow-hidden(ref='swiperBox')
  .swiper-wrapper
    .swiper-slide.justify-center
      img.mb-10.w-50vmin(src='~/assets/img/mine.png', class='rounded-1/2')
      ...
      .flex.justify-around(class='w-1/2')
        ...
        span.relative
          ...
          img.absolute.w-20vw.translate-y-full.w-20vw.-bottom-10px.max-w-unset(src='~/assets/img/we-chat.jpg', alt='we-chat', ref='weChatImg', v-show='showWeChat', class='-translate-x-1/2 left-1/2')
        a(href='http://wpa.qq.com/msgrd?v=3&uin=512060925&site=qq&menu=yes')
          IconQq.text-3xl(class='text-#1594d3')
    .swiper-slide(class='pt-1/5')
      h1.text-3xl.mb-8 个人技能
      .flex.flex-col.w-77vw
        .card(:class='{ hide: showIndex !== i }', v-for='(txt, i) in skillsText.filter((v, j) => j < skillsText.length / 2)', @click='showIndex = i')
